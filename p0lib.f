...
c p0lib.f contains basic communications procedures for 1d partitions:
...
c PPINIT initializes parallel processing, returns porcessor ids,
c        creates power of 2 sub communicator, and returns its size.
c PPID gets processor ids and number of processors.
c PPINIT0 initializes parallel processing for Fortran77, requests nvp
c         processors, and returns processor id
c PPINIT1 initializes parallel processing for Fortran90, returns
c         number of processors and processor id.
c PPEXIT terminates parallel processing.
c PPABORT aborts parallel processing.
c HARTBEAT procedure for nodes in main communicator to send a message to
c          diagnostic nodes.
c PTIMERA performs synchronized parallel wall clock timing.
c PWTIMERA performs parallel local wall clock timing.
c PSUM performs parallel sum of a real vector, with length a power of 2.
c PISUM performs parallel sum of an integer vector, with length a power
c       of 2.
c PCSUM performs parallel sum of a complex vector, with length a power
c       of 2.
c PDSUM performs parallel sum of a double precision vector, with length
c       a power of 2.
c PMAX performs parallel maximum of a real vector, with length a power
c      of 2.
c PIMAX performs parallel maximum of an integer vector, with length a
c       power of 2.
c PDMAX performs parallel maximum of a double precision vector, with
c       length a power of 2.
c PSCAN performs a parallel prefix reduction of a real vector, with
c       length a power of 2.
c PISCAN performs a parallel prefix reduction of an integer vector, with
c        length a power of 2.
c PDSCAN performs a parallel prefix reduction of a double precision
c        vector, with length a power of 2.
c PBCAST broadcasts real data.
c PBICAST broadcasts integer data.
c PBCCAST broadcasts complex data.
c PBDCAST broadcasts double precision data.
c P0COPY copies real data on node 0 to other nodes.
c PWRITE1 collects distributed real data and writes to a  direct access
c         binary file.
c PREAD1 reads real data from a direct access binary file and
c        distributes it.
c PWRDATA collects distributed real data and writes it to a fortran
c         unformatted sequential file.
c PRDDATA reads data from a fortran unformatted sequential file and
c         distributes it.
c PWRPART collects distributed particle data and writes it to a fortran
c         unformatted sequential file.
c PRDPART reads particle data from a fortran unformatted sequential file
...
c this subroutine broadcasts complex data f
c f = data to be broadcast
c nxp = size of data f
c input: f, nxp
c output: f
...
c first send data to remaining nodes
         do 10 i = 2, np
            id = i - ioff
            call MPI_SEND(f,nxp,mcplx,id,97,lworld,ierr)
   10    continue
c then send data to node 0
         if (ioff.eq.0) then
            id = 1
...
c other nodes receive data from node 0
      elseif (idproc.le.nproc) then
         call MPI_RECV(f,nxp,mcplx,0,97,lworld,istatus,ierr)
      endif
      return
      end
c-----------------------------------------------------------------------
