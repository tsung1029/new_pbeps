...
!     integer :: npxy, npxyb, np, npxyi, npxybi, npi
      ...
      real :: qbme, qbmi, affp, dth, qi0, omt, q2m0, wp0, etx
      ...
      complex, dimension(:,:,:,:), pointer :: cut, amut, fxyt, bxyt
      ...
      real, dimension(:,:,:,:), pointer :: cui, cutt, vfield
...
! set default diagnostic file names
      ...
      if (nte > 0) fename = 'pvpotrk2.'//cdrun
! energy time history
      if (ndw > 0) then
...
! in real space, cu(3,j+1,k,l) = current density at grid point (j,kk)
      allocate(cu(3,nxe,nypmx*kbmin,kblok))
...
!           if (npxyi > 0) call distr(parti,edges,nppi,nps,vtxi,vtyi,   &
!    &vtzi,vxi0,vyi0,vzi0,npxi,npyi,nx,ny,ipbc)
            if (npxyi > 0) then
...
!        call retardg(part,npp,dth,ci,nx,ny,ipbc,relativity)
! move electrons into appropriate spatial regions
!        call pmove(part,edges,npp,tmove,ny,kstrt,nvp,nbmax,vect,ierr)
! retard ion velocities to deposit current
!        if (movion==1) then
!           call retardg(parti,nppi,dth,ci,nx,ny,ipbc,relativity)
! move ions into appropriate spatial regions
!           call pmove(parti,edges,nppi,tmovi,ny,kstrt,nvp,nbmax,vect,  &
!    &ierr)
!        endif
...
! read restart files
     ...
     &movion,nppi,parti,qi,irc,iuer,ef=exyz,bf=bxyz)
         if (irc /= 0) go to 400
...
! handle error
         if (irc /= 0) then
            if (id0==0) write (iuer,*) 'Restart Error'
            go to 3000
         endif
         if (irc /= 0) go to 400
         ntime = itime + itime0
         ...
         go to 490
! handle error
  400    if (id0==0) write (iuer,*) 'Restart Error'
         go to 3000
      endif
!
  490 if (ntime > 0) dth = .5*dt
! sorting arrays
...
! ion current diagnostics
     ...
     &jblok,iuj,njrec,fjname)
      if (ntj > 0) then
         allocate(cui(3,nxe,nypmx*kbmin,kblok))
         allocate(cutt(3,nxe,nypmx*kbmin,kblok))
         cui = 0.0; cutt = 0.0
         if (id0==0) then
            ceng = zero
...
! electromagnetic diagnostics
      call initvmodediag(vpotr,nte,id0,nxh,nyh,kxp,ndim,modesxe,modesye,&
     &jblok,iue,nerec,fename)
      if (nte > 0) then
         if (id0==0) then
            ceng = zero
            write (iudm,pem2d,iostat=irc)
         endif
      endif
...
! save electron current for ion current diagnostic
      call vpotrdiagprep(cu,cutt,ntj,ndj,ntime)
! add ion current and deposit ion charge density
      if (movion==1) then
         call djpostg(parti,cu,nppi,noff,qmi,dth,ci,tdjposti,nx,ny,ipbc,&
     &relativity,inorder,djopt)
! move ions into appropriate spatial regions
...
! save ion current
      call vpotrdiagprep(cu,cutt,ntj,ndj,ntime,diff=.true.)
...
! electromagnetic diagnostic
      call vpotrdiag(bxyz,cut,vfield,vpotr,vfieldt,ffc,nyp,mixup,sct,   &
     &tfft,affp,ci,nte,nde,nx,ny,modesxe,modesye,iue,nerec,indx,indy,   &
     &ntime,nvp,kstrt,kxp,kyp,ndstyle,irc,inorder)
      if (irc==1) go to 2000
! ion current diagnostic
      call avcurdiag(cutt,cui,fxyt,vfield,vcurt,vfieldt,ffc,nyp,mixup,  &
     &sct,tfft,ntj,ndj,nx,ny,modesxj,modesyj,iuj,njrec,indx,indy,ntime, &
     &nvp,kstrt,kxp,kyp,ngds,ndstyle,irc,inorder)
      if (irc==1) go to 2000
...
! calculate initial darwin electric field
         allocate(amu(4,nxe,nypmx*kbmin,kblok),amut(4,nyv,kxp,jblok))
         allocate(ffe(nyh,kxp,jblok))
! deposit momentum flux
         call sguard(amu,nyp,zero,zero,zero,zero,nx,inorder)
         call dmjpostg(part,amu,npp,noff,qme,ci,ts,relativity,inorder,  &
     &djopt)
         if (movion==1) then
            call dmjpostg(parti,amu,nppi,noff,qmi,ci,ts,relativity,     &
     &inorder,djopt)
         endif
...
! solve for darwin electric field
     ...
     &inorder)
         bxyt = cut
         call dcuperp(bxyt,amut,nx,ny,kstrt)
         call epois_init(ffe,ax,ay,affp,wp0,ci,nx,ny,kstrt)
         call iepois(bxyt,exyz,ffe,affp,ci,wf,tfield,nx,ny,kstrt)
         deallocate(amu,amut,ffe)
         dth = .5*dt
! calculate electromagnetic fields
      else
         call maxwel(exyz,bxyz,cut,ffc,affp,ci,dt,wf,wm,tfield,nx,ny,   &
     &kstrt)
      endif
! vector potential diagnostic
      call avpotdiag(bxyz,vfield,vpott,vfieldt,nyp,mixup,sct,tfft,nta,  &
     &nda,nx,ny,modesxa,modesya,iua,narec,indx,indy,ntime,nvp,kstrt,kxp,&
     &kyp,ndstyle,irc,inorder)
      if (irc==1) go to 2000
...
! particle push and charge density update
      wke = 0.
! push electrons
      call push3g(part,fxyze,bxyze,npp,noff,qbme,dt,dth,ci,wke,tpush,nx,&
...
! push ions
      if (movion==1) then
         wki = 0.
         call push3g(parti,fxyze,bxyze,nppi,noff,qbmi,dt,dth,ci,wki,    &
...
! calculate electron momentum
      call fmomtdiag(part,qt,ffc,exyz,bxyz,npp,msg,affp,pxe,pye,pze,sx, &
     &sy,sz,wx,wy,wz,ntm,id0,ium,kstrt,nx,ny,ntime)
! calculate ion and total momentum
      call imomtdiag(parti,qi,fxyze,nppi,nyp,msg,dt,rmass,pxi,pyi,pzi,wx&
...
! energy diagnostic
      call emenergy(wt,wtot,msg,we,wf,wm,wke,wki,ntw,ndw,id0,itw,iuot,  &
     &ntime)
      itime = itime + 1
      ntime = itime + itime0
! restart file
...
! send main CPU Time to diagnostic nodes
      ...
      msg(6) = tsort; msg(7:8) = tmove; msg(9:10) = tfft
      msg(11) = tfield; msg(12) = ltime
...
! electromagnetic diagnostics
         if (nte > 0) then
            ...
            ceng = affp
            write (iudm,pem2d,iostat=irc)
            if (irc /= 0) write (iuer,*) 'pem2d namelist not written'
      endif
...
! allocate data for restart and/or phase space diagnostic
         ...
         if ((nda > 0) .or. (ndj > 0) .or. (nde > 0)) then
            allocate(vfield(ndim,nxe,nypmx*kbmin,kblok))
            vfield = 0.0
         endif
! restart
     ...
     &part,movion,nppi,parti,qi,irc,iuer,ef=exyz,bf=bxyz)
            if (irc /= 0) go to 30
! extend run
            if (nustrt==0) then
               itime0 = itime + itime0
...
! get main CPU Time
            ...
            tfft = msg(9:10); msg(11) = tfield; msg(12) = ltime
            if (id0==0) then
               write (iuot,*) 'processor partition used: nvp = ', nvp
     ...
     &tdjpost, 'sec'
            ...
            if (movion==1) then
               call HARTBEAT(msg,6)
               tpushi = msg(1); tdposti = msg(2); ; msg(3) = tdjposti
               tsorti = msg(4); tmovi = msg(5:6)
               if (id0==0) then
                  totpushi = tpushi + tdposti + tdjposti
                  write (iuot,*) 'ion push time=', tpushi, 'sec'
...
! electromagnetic diagnostic
         if ((nte > 0) .or. (nde > 0)) then
            ...
            nt = 2*modesye - 1
            if (it==0) call writebf(vpotr,modesxe,nt,kxp,iue,nerec)
...
! write file
     ...
     &movion,nppi,parti,qi,ef=exyz,bf=bxyz)
               call restart_dwrite(it,id0,itime,itw,wt,ndrec,fdname,    &
     &nprec,fpname,narec,faname,njrec,fjname,nerec,fename)
            endif
         endif
         call wtimer(tloop,ldtime)
         ltime = ltime + tloop
